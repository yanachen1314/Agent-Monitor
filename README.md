...
检查更新时，客户端会先下载 `latest.yml`。确认下载后，客户端优先读取新版本 `.blockmap`，并从本地缓存
读取旧版本 blockmap；缓存不存在时会再下载旧版本 `.blockmap`。随后仅下载新安装包中发生变化的数据块，
在本地缓存目录重建完整的 NSIS `.exe` 安装包；如果差分下载不可用，则回退为下载完整安装包。自动更新不会
下载 `win-unpacked.zip`、`SHA256SUMS.txt` 或 GitHub 自动生成的源码压缩包。
