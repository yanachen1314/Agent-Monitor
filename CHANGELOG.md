...
`0.4.2` 改进了应用内更新说明的展示与链接安全：

- 更新说明改为 Markdown 渲染，并兼容 GitHub Release 返回的 HTML 内容，不再直接显示标签源码。
- 更新说明区域最大高度限制为 220px，内容较多时可在区域内纵向滚动。
- 补充标题、列表、引用、行内代码、代码块和链接样式，使更新内容更易阅读。
- 更新说明会在渲染前清理危险标签和属性；外部链接仅允许无凭据的 `http/https` 地址，并通过
  系统默认浏览器打开。
- 新增 Markdown 清理与外部链接校验测试。
